                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-3 py-4 text-sm">
                      ...
                      <div className="text-gray-900">
                        ...
                        <span className="text-gray-500 ml-1">Comb:</span>{" "}
                        {vehicle.combustible}
                      </div>
                    </td>
          ...
          <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                de <span className="font-medium">{vehiculos.length}</span>{" "}
                vehículos
              </p>
            </div>
            <div>
              <div className="flex space-x-2">
                ...
                <button
                  className={`px-3 py-1 border rounded-md ${
                    page === Math.ceil(vehiculos.length / itemsPerPage)
                      ? "bg-gray-100 text-gray-400 cursor-not-allowed"
                      : "bg-white text-gray-700 hover:bg-gray-50"
                  }`}
                  disabled={page === Math.ceil(vehiculos.length / itemsPerPage)}
                  onClick={() => cambiarPagina(page + 1)}
                >
                  Siguiente
                </button>
...
const TableDocumentInfo = ({ vehicle }: { vehicle: Vehiculo }) => {
  ...
  return (
    <div>
      ...
      <span
        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusIndicator()}`}
      >
        {getSummaryText()}
      </span>
      ...
      {priorityDoc && (
        <div className="mt-1 text-xs">
          <span
            className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium ${getStatusColor(priorityDoc.status)}`}
          >
            {priorityDoc.name}:{" "}
